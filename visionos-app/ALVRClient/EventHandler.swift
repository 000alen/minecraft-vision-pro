...
class EventHandler: ObservableObject {
    ...
    func handleMdnsBroadcasts() {
        if mdnsListener != nil && mdnsListenerRegistered && mdnsListenerReady && !streamingActive {
    ...
    func handleNals(frameData: AlvrVideoFrameData) -> Bool {
        ...
        if self.vtDecompressionSession == nil {
            let fallbackCodec = currentCodec == -1 ? Int(ALVR_CODEC_HEVC.rawValue) : currentCodec
            print("Missing decoder config; trying to create decoder from video NALs with codec \(fallbackCodec)")
            (vtDecompressionSession, videoFormat) = VideoHandler.createVideoDecoder(initialNals: nal, codec: fallbackCodec)
            currentCodec = fallbackCodec
        }

        if let vtDecompressionSession = self.vtDecompressionSession, let videoFormat = self.videoFormat {
            VideoHandler.feedVideoIntoDecoder(decompressionSession: vtDecompressionSession, nals: nal, timestamp: timestamp, videoFormat: videoFormat, codec: currentCodec) { [self] imageBuffer in
