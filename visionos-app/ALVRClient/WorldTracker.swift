...
class WorldTracker {
    ...
    func initializeAr() async  {
        ...
        var authorizationTypes: [ARKitSession.AuthorizationType] = [.handTracking, .worldSensing]
#if XCODE_BETA_26 && VISIONCRAFT_ENABLE_ACCESSORY_TRACKING
        if #available(visionOS 26.0, *) {
            authorizationTypes.append(.accessoryTracking)
        }
#endif
        var authStatus: [ARKitSession.AuthorizationType : ARKitSession.AuthorizationStatus] = [:]
        authStatus = await arSession.requestAuthorization(for: authorizationTypes)
        ...
        do {
            ...
            DispatchQueue.main.async {
                EventHandler.shared.connectionFlavorText = "ARKit failed to start. Check world/hand tracking permissions, then relaunch ALVRClient."
            }
            objc_sync_exit(arkitLock)
            return
        }
